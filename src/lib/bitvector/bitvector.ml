...
module U = Hashtbl.Make(struct type t = int let equal = (=) let hash x = x end)
let units = U.create 128

let ulim s = if s < 62 then 1 lsl s |> Bigint.big_int_of_int else fst (U.find units s)
let slim s = ulim (s - 1)

module Internal :
sig
  type internal = private { value: Bigint.t; size: int }

  val create : Bigint.t -> int-> internal
end =
struct
  type internal = { value: Bigint.t; size: int }

  let create value size =
    if size <= 0 then invalid_arg "Negative bitvector size";
    let mask = if size < 62 then
                 1 lsl size - 1 |> Bigint.big_int_of_int
               else begin
                   let size' = size - 1 in
                   if not (U.mem units size') then begin
                       let slimit = Bigint.(shift_left_big_int unit_big_int size') in
                       let mask' = Bigint.(sub_big_int slimit unit_big_int) in
                       U.add units size' (slimit, mask') end;
                   match U.find units size with
                   | _, mask -> mask
                   | exception Not_found ->
                      let ulimit = Bigint.(shift_left_big_int unit_big_int size) in
                      let mask = Bigint.(sub_big_int ulimit unit_big_int) in
                      U.add units size (ulimit, mask);
                      mask
                 end in
    let value  = Bigint.(and_big_int value mask) in
    { value; size }
end
...
let signed_of { Internal.value; size } =
  if Bigint.lt_big_int value (slim size) then value
  else Bigint.sub_big_int value (ulim size)
...
let print_dec bv =
  Printf.sprintf "%s" (Bigint.string_of_big_int bv.value)
...
let of_int i sz = create (Bigint.big_int_of_int i) sz
...
let to_float bv  = Bigint.float_of_big_int bv.value
